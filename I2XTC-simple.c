...
#endif
#define FRAMES_PER_BUFFER 50000
#define MONO 1
#define STEREO 2
...
float leftOLA[FRAMES_PER_BUFFER/2];
float rightOLA[FRAMES_PER_BUFFER/2];
float *H30LHRTF;
float *H30RHRTF;
float *H330LHRTF;
float *H330RHRTF;
float test[FRAMES_PER_BUFFER/2+511];
float counter = 0.f;
...
int main(int argc, char **argv)
{
	...
	printf("H30L audio file: Frames: %d Channels: %d Sample Rate: %d\n",
		(int)data.H30Linfo.frames, data.H30Linfo.channels, data.H30Linfo.samplerate);
	printf("H30R audio file: Frames: %d Channels: %d Sample Rate: %d\n",
		(int)data.H30Rinfo.frames, data.H30Rinfo.channels, data.H30Rinfo.samplerate);
	printf("H330L audio file: Frames: %d Channels: %d Sample Rate: %d\n",
		(int)data.H330Linfo.frames, data.H330Linfo.channels, data.H330Linfo.samplerate);
	printf("H330R audio file: Frames: %d Channels: %d Sample Rate: %d\n",
		(int)data.H330Rinfo.frames, data.H330Rinfo.channels, data.H330Rinfo.samplerate);
	...
	//calc rms
	for (int i = 0; i < FRAMES_PER_BUFFER/2+511; i++)
	{
		counter += test[i]*test[i];
	}
	counter = counter / (float)(FRAMES_PER_BUFFER/2+511);
				...
				void *userData)
{
	...
	//add delay and scale amplitude of HRTFs
	if (data->delay < 0)
	{
	...
	//fftw variables
	fftw_complex *in_fft_l, *out_fft_l, *in_fft_r, *out_fft_r, *crosstalk_IR_L, *crosstalk_IR_R;
	...
	{
		crosstalk_IR_L[i][0] = 0.f;
		crosstalk_IR_R[i][0] = 0.f;
		leftOLA[i] = 0.f;
		rightOLA[i] = 0.f;
		crosstalk_IR_L[i][1] = 0.f;
		crosstalk_IR_R[i][1] = 0.f;
	}
	...
	//zero pad HRTF complex buffers manually
	...
	//process HRTFs, convolve HRTFS with complex fft output
	for (int i = 0; i < framesPerBuffer+data->H30Linfo.frames-1; i++)
	{
		//crosstalk_IR
		x.a = fftw_H330R_out[i][0];
		...
		// if (callbackcount == 1 && i == 1)
		// test[i] = crosstalk_IR_L[i][0];
	...
	for (int i = 0; i < framesPerBuffer; i++)
	{
		...
		if (callbackcount == 20)
		test[i] = ((float)in_fft_l[i][0]/(float)(framesPerBuffer+data->H30Linfo.frames-1) + leftOLA[i])/2.0f;
	}

	//normalize and copy to output buffer
	// for (int i = 0; i < framesPerBuffer; i++)
	// {
	// 	outbuff[2*i] = (float)in_fft_l[i][0]/(float)(framesPerBuffer+data->H30Linfo.frames-1);
	// 	outbuff[2*i+1] = (float)in_fft_r[i][0]/(float)(framesPerBuffer+data->H30Linfo.frames-1);
	// 	if (callbackcount == 20)
	// 	test[i] = (float)in_fft_l[i][0]/(float)(framesPerBuffer+data->H30Linfo.frames-1);
	// }
